...
?>
<div class="doc-page tpl-premium <?= $lang === 'ta' ? 'lang-ta' : '' ?>" style="<?= $idx > 0 ? 'margin-top:40px; page-break-before:always;' : '' ?>">
  <?php
    // Watermark: use custom text if set, else fall back to company name when enabled.
    $wm_text = !empty($watermark_text) ? $watermark_text
             : (function_exists('mb_strtoupper') ? mb_strtoupper(mb_substr($company['name'], 0, 12)) : strtoupper(substr($company['name'], 0, 12)));
    $wm_show = (bool)$watermark_enabled;
  ?>
  <?php if ($wm_show): ?>
  <div class="doc-watermark"><?= h($wm_text) ?></div>
  <?php endif; ?>
  ...
  <table class="doc-table">
    ...
    <tbody>
      ...
      <tr>
        ...
        <?php if ($show_col_desc): ?><td><b><?= h($it['name']) ?></b></td><?php endif; ?>
  ...
  <div class="doc-footer-grid">
    ...
    <div class="doc-bank">
      ...
      <?= h($company['bank_name']) ?> · <?= h($company['bank_account']) ?><br>IFSC: <?= h($company['bank_ifsc']) ?>
    </div>
    <?php endif; ?>
    <div class="doc-sign">
